...
The solution is quite clever (special thanks to [Wisha](https://wisha.page/) for helping me figure
it out) - we tag every allocation with a generation number.
...
I secretly made two different garbage collector implementations - one which is thread-local, saving
...
I implemented both a single-threaded and multit-threaded version of this benchmark, and ran every
garbage collector through the single-threaded one and the concurrent ones through the multi-threaded
one.
