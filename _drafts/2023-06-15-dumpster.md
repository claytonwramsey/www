...
of determining whether allocations reachable from the original one - if the original allocation
...
reference graph, recording a pointer to the allocation and its implementation of `drop`.
Then, once we've found every inaccessible allocation, we'll drop them all in one quick go.
...
As a result, my implementation actually has two different kinds of erased pointers:
...
Here's a simplified example of how it's used:
...
force every single operation involving a `Gc` to acquire a single global mutex, preventing any sort
of concurrency bugs at all.
